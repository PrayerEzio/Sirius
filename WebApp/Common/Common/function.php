 ...
 * @return string 
 */
function re_md5($str,$controller = MODULE_NAME)
{
	$key = C('WKY_KEY');
	return '' === $str ? '' : hash('sha256',md5(sha1($str).$key).$controller);
}
/**
 * 格式化价格
 * @param  string $price 
 ...
 */
 ...
 function de_replaceEmoji($str)
 {
 	$text = preg_replace("#\\\u([0-9a-f]+)#ie","iconv('UCS-2','UTF-8', pack('H4', '\\1'))",$str);
 	return $text;
 }
 function get_member_nickname($member_id){
 	$member = M('Member')->where(array('memebr_id'=>$member_id))->field('member_name,nickname')->find();
 ...
 function get_seller_nickname($member_id){
 	$seller = M('Seller')->where(array('seller_id'=>$member_id))->field('seller_name,nickname,mobile')->find();
 	if (!empty($seller['nickname'])) {
 		return $seller['nickname'];
 	}elseif (!empty($seller['seller_name'])){
 ...
 function get_handle_status_name($handle_status){
 ...
 function vip_level($mid,$table = 'Seller'){
 	$where[strtolower($table).'_id'] = $mid;
 	$where['start_time'] = array('lt',NOW_TIME);
 	$where['end_time'] = array('gt',NOW_TIME);
 	return M($table.'Vip')->where($where)->getField('vip_level');
 }